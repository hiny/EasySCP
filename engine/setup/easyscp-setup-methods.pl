...
die("Unable to create EasySCP log directory $!\n") unless $rs == 0;
...
################################################################################
# Ask for EasySCP Frontend vhost name
#
# @return int 0 on success, -1 otherwise
#
sub ask_vhost {
...
sub ask_db_name {
	...
	$main::ua{'db_name'} = ($rdata eq '') ? 'easyscp' : $rdata;

	push_el(\@main::el, 'ask_db_name()', 'Ending...');
}

################################################################################
# Ask for EasySCP SQL user
#
# @return void
...
sub ask_db_ftp_password {
	...
	if (!defined $pass1  || $pass1 eq '') {
		$dbPassword = generateRandomPass(18);
...
################################################################################
# Ask for EasySCP Frontend first admin name
#
# @return void
...
################################################################################
# Ask for EasySCP Frontend first admin password
#
# @return int 0 on success, -1 otherwise
#
...
################################################################################
# Ask for EasySCP Frontend first admin email
#
# @return int 0 on success, -1 otherwise
#
sub ask_admin_email {
...
sub ask_db_pma_password {
	...
	if (!defined $pass1 || $pass1 eq '') {
		my $dbPassword = generateRandomPass(18);
	...
	if(!defined $hostname) {
		...
		if(!defined $hostname or
			...
			if(getCmdExitValue() != 0) {
				exit_msg(
					-1, colored(['bold red'], "[ERROR] ") . 'Unable to find ' .
...
# called after the EasySCP configuration file processing (loading, updating...).
#
# Note:
#
...
# Starting services
#
# This subroutine start all services managed by EasySCP and that are not marked as
# 'no' in the main EasySCP configuration file (easyscp.conf).
#
sub start_services {
	...
	for (
		qw/CMD_EASYSCPN CMD_EASYSCPD CMD_NAMED CMD_HTTPD CMD_FTPD CMD_CLAMD
...
# Stopping services
#
# This subroutines stop all the services managed by EasySCP.
#
sub stop_services {
...
# - Delete empty files in EasySCP configuration directories
#
...
sub setup_resolver {
	...
	if(-e $main::cfg{'RESOLVER_CONF_FILE'}) {
		...
		$main::exitMessage = colored(['bold red'], "\n\t[ERROR] ") .
			"Unable to find your resolv.conf file!\n";
		return -1;
	}
...
################################################################################
# EasySCP crontab file - (Setup / Update)
#
# This subroutine built, store and install the EasySCP crontab file
#
sub setup_crontab {
	...
	if ($main::cfg{'ROOT_GROUP'} eq 'wheel') {
		$prodDir = '/usr/local/etc/easyscp/cron.d';
	} else {
		$prodDir = '/etc/cron.d';
	}

	# Saving the current production file if it exists
	if(-e "$prodDir/easyscp") {
		$rs = sys_command(
			"$main::cfg{'CMD_CP'} -p $prodDir/easyscp $bkpDir/easyscp." . time
		);
		return $rs if ($rs != 0);
	}
	...
	# Loading the template from /etc/easyscp/cron.d/easyscp
	($rs, $cfgTpl) = get_file("$cfgDir/easyscp");
	return $rs if ($rs != 0);

	# Awstats cron task preparation (On|Off) according status in easyscp.conf
	...
	$rs = store_file(
		"$wrkDir/easyscp", $$cfg, $main::cfg{'ROOT_USER'},
		$main::cfg{'ROOT_GROUP'}, 0644
	);
	return $rs if ($rs != 0);

	# Install the new file in production directory
	$rs = sys_command("$main::cfg{'CMD_CP'} -fp $wrkDir/easyscp $prodDir/");
	return $rs if ($rs != 0);
...
sub setup_fcgi_modules {
	...
	for (qw/fcgid_easyscp.conf fcgid_easyscp.load/) {
	...
	# Loading the template from the /etc/easyscp/apache directory
	($rs, $cfgTpl) = get_file("$cfgDir/fcgid_easyscp.conf");
	return $rs if ($rs != 0);
	...
	# Storing the new file
	$rs = store_file(
		"$wrkDir/fcgid_easyscp.conf", $$cfg, $main::cfg{'ROOT_USER'},
		$main::cfg{'ROOT_GROUP'}, 0644
	);
	return $rs if ($rs != 0);
	...
	$rs = sys_command(
		"$main::cfg{'CMD_CP'} -pf $wrkDir/fcgid_easyscp.conf " .
		"$main::cfg{'APACHE_MODS_DIR'}/"
	);
	return $rs if($rs != 0);

	# Loading the system configuration file
        ($rs, $$cfg) = get_file("$main::cfg{'APACHE_MODS_DIR'}/$main::cfg{'PHP_FCGID_CONF_FILE'}"); 
	return $rs if ($rs != 0);

	# Building the new configuration file
	$$cfg = "<IfModule !mod_fcgid.c>\n" . $$cfg . "</IfModule>\n";

	# Store the new file
	$rs = store_file(
		"$wrkDir/fcgid_easyscp.load", $$cfg, $main::cfg{'ROOT_USER'},
		$main::cfg{'ROOT_GROUP'}, 0644
	);
	return $rs if ($rs != 0);
	...
	$rs = sys_command(
		"$main::cfg{'CMD_CP'} -pf $wrkDir/fcgid_easyscp.load " .
		"$main::cfg{'APACHE_MODS_DIR'}/"
	);
	return $rs if($rs != 0);
...
################################################################################
# EasySCP httpd main vhost - (Setup / Update)
#
# This subroutine do the following tasks:
#  - Built, store and install EasySCP main vhost configuration file
...
sub setup_httpd_main_vhost {
	...
	# Saving the current production file if it exists
	if(-e "$main::cfg{'APACHE_SITES_DIR'}/easyscp.conf") {
		my $rs = sys_command(
			"$main::cfg{'CMD_CP'} -p $main::cfg{'APACHE_SITES_DIR'}/" .
			"easyscp.conf $bkpDir/easyscp.conf.". time
		);
		return $rs if($rs != 0);
	}
	...
	# Loading the template from /etc/easyscp/apache/
	...
	$rs = store_file(
		"$wrkDir/easyscp.conf", $$cfg, $main::cfg{'ROOT_USER'},
		$main::cfg{'ROOT_GROUP'}, 0644
	);
	return $rs if ($rs != 0);
	...
	$rs = sys_command(
		"$main::cfg{'CMD_CP'} -pf $wrkDir/easyscp.conf " .
	...
	if(! -e '/etc/SuSE-release' && -e '/usr/sbin/a2enmod') {
		...
		$rs = sys_command("/usr/sbin/a2ensite easyscp.conf");
		return $rs if($rs != 0);
	}
...
################################################################################
# EasySCP awstats vhost - (Setup / Update)
#
# This subroutine do the following tasks:
...
sub setup_awstats_vhost {
	...
	if ($main::cfg{'AWSTATS_ACTIVE'} eq 'yes' && $main::cfg{'AWSTATS_MODE'} eq 0) {
		...
		# update it with the Awstats information. If not, use the EasySCP file.
...
sub setup_mta {
	...
	($rs, $$cfg) = prep_tpl(
		{
			...
			'{ARPL_PATH}'	=> $main::cfg{'ROOT_DIR'}."/engine/messenger/easyscp-arpl-msgr",
		},
		$cfgTpl
	);
	return $rs if ($rs != 0);
	...
	$rs = setfmode(
		"$main::cfg{'ROOT_DIR'}/engine/messenger/easyscp-arpl-msgr",
...
sub setup_ftpd {
	...
	# Update:
		...
		} elsif(-e "$main::cfg{'CONF_DIR'}/proftpd/backup/proftpd.conf.easyscp") {
			$wrkFile = "$main::cfg{'CONF_DIR'}/proftpd/backup/proftpd.conf.easyscp";
		} elsif(-e '/etc/proftpd.conf.bak') {
			$wrkFile = '/etc/proftpd.conf.bak';
		}

		# Loading working configuration file from /etc/easyscp/working/
	...
	# Loading the template from /etc/easyscp/proftpd/
...
################################################################################
# EasySCP Daemon, network - (Setup / Update)
#
# This subroutine install or update the EasySCP daemon and network init scripts
#
# @return int 0 on success, other on failure
#
sub setup_easyscp_daemon_network {

	push_el(\@main::el, 'setup_easyscp_daemon_network()', 'Starting...');

	my ($rs, $rdata, $fileName);

	for ($main::cfg{'CMD_EASYSCPN'}, $main::cfg{'CMD_EASYSCPD'}) {
		...
		if(-x '/usr/sbin/update-rc.d') {
			...
			# easyscp_network should be stopped before the MySQL server (due to the
			# interfaces deletion process)
			if($fileName eq 'easyscp_network') {
	...
	push_el(\@main::el, 'setup_easyscp_daemon_network()', 'Ending...');

	0;
}

################################################################################
# EasySCP GUI apache vhost - (Setup / Update)
#
# This subroutine built, store and install EasySCP GUI vhost configuration file.
#
# @return int 0 on success, other on failure
#
sub setup_gui_httpd {
...
################################################################################
# EasySCP GUI PHP configuration files - (Setup / Update)
#
# This subroutine do the following tasks:
...
sub setup_gui_php {
	...
	# Loading the template from /etc/easyscp/fcgi/parts/master
	...
	# Loading the template from /etc/easyscp/fcgi/parts/master/php5
...
################################################################################
# EasySCP GUI pma configuration file and pma SQL control user - (Setup / Update)
#
...
sub setup_gui_pma {
	...
	# Update recovery
		...
		print colored(['bold yellow'], "\n\n\tWARNING: ") .
			"Unable to find your working PMA configuration file !\n" .
	...
	if (defined $main::ua{'db_pma_user'}) {
		...
		# Update the easyscp.conf file, reset the DSN and force reconnection on
		# the next query
...
# @return int 0 on success, -1 otherwise
#
sub setup_gui_roundcube {

	push_el(\@main::el, 'setup_gui_roundcube()', 'Starting...');

	my $cfgDir = "$main::cfg{'CONF_DIR'}/roundcube";
	my $prodDir = "$main::cfg{'GUI_ROOT_DIR'}/tools/webmail";

	# Converting to ASCII (Punycode)
	my $dbHost = idn_to_ascii($main::cfg{'DATABASE_HOST'}, 'utf-8');

	my ($rs, $cubeUser, $cubeUserPwd, $cfgFile);

	# Setup:
	$cubeUser = "cube";
	$cubeUserPwd = generateRandomPass(18);

	## Building the new file

	# Getting the template file for DB
	($rs, $cfgFile) = get_file("$cfgDir/db.inc.tpl");
	return -1 if ($rs != 0);
	...
	# Note: permission are set by the set-gui-permissions.sh script
	$rs = store_file(
		"$prodDir/config/db.inc.php", $cfgFile, "$main::cfg{'ROOT_USER'}",
		"$main::cfg{'ROOT_GROUP'}", 0640
	);
	return -1 if ($rs != 0);

	# Getting the template file for MAIN
	($rs, $cfgFile) = get_file("$cfgDir/main.inc.tpl");
	return -1 if ($rs != 0);

	# Storing the file in the working directory
	# Note: permission are set by the set-gui-permissions.sh script
	$rs = store_file(
		"$prodDir/config/main.inc.php", $cfgFile, "$main::cfg{'ROOT_USER'}",
		"$main::cfg{'ROOT_GROUP'}", 0640
	);
	return -1 if ($rs != 0);

	## Creating RoundCube control user account if needed
	...
	# Sets the rights for the roundcube user
	($rs) = doSQL(
		qq/
			GRANT ALL PRIVILEGES ON `roundcubemail`.*
			TO '$cubeUser'\@'$dbHost'
			IDENTIFIED BY '$cubeUserPwd' ;
		/
	);
	return -1 if ($rs != 0);
...
################################################################################
# EasySCP update/correct GUI theme, if current theme setting not possible
#
# @return int 0 on success, other on failure
#
...
sub setup_gui_named_cfg_data {
	...
	# Loading all needed templates from /etc/easyscp/bind/parts
	...
	# Loading working file from /etc/easyscp/bind/working/named.conf
...
################################################################################
# EasySCP Gui named dns record's - (Setup / Update)
#
...
sub setup_gui_named_db_data {
	...
	# Loading the template from /etc/easyscp/bind/parts
...
sub setup_rkhunter {
	...
	# to disable the default cron task (EasySCP provides its own cron job for
	# rkhunter)
...
sub setup_services_cfg {
	...
	if(defined &setup_engine) {
		...
		if (!defined($ARGV[0]) || $ARGV[0] ne '-rpm') {
			subtitle('EasySCP users and groups:');
	...
	for (
		...
		[\&setup_po, 'EasySCP Courier-Authentication:'],
		[\&setup_ftpd, 'EasySCP ProFTPd configuration file:'],
		[\&setup_easyscp_daemon_network, 'EasySCP init scripts:']
	) {
		subtitle($_->[1]);
		print_status(&{$_->[0]}, 'exit_on_error');
	}
...
sub setup_gui_cfg {
	...
	for (
		...
		[\&setup_gui_php, 'EasySCP GUI fastCGI/PHP configuration:'],
...
sub additional_tasks{
	...
	subtitle('EasySCP Rkhunter configuration:');
	my $rs = setup_rkhunter();
	print_status($rs);

	subtitle('EasySCP System cleanup:');
	system_cleanup();
	print_status(0);
