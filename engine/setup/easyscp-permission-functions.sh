...
# read needed entries from easyscp.conf
CONF_FILE="/etc/easyscp/easyscp.conf"
if [ -f /usr/local/etc/easyscp/easyscp.conf ]
then
    CONF_FILE="/usr/local/etc/easyscp/easyscp.conf"
fi

OLD_IFS=$IFS
IFS=$

# Reading needed entries from easyscp.conf
