...
$main::cc_stdout = '/tmp/easyscp-cc.stdout';

$main::cc_stderr = '/tmp/easyscp-cc.stderr';
...
$main::master_name = 'easyscp-rqst-mngr';
...
sub set_zone {
	...
	while(length($fdata) > 0) {
		...
		if($zs == 0) {
			if(index($curline, $comment."## START EasySCP ".$zone." ###") == 0 ) {
				$zs = 1;
			} else {
				$bz .= $curline;
			}
		} elsif($ze == 0) {
			if(index($curline, $comment."## END EasySCP ".$zone." ###") == 0) {
				$ze = 1;
			}
		} elsif($ze == 1) {
			$az .= $curline;
	...
	return
		$bz . ($zs == 1 ? "" : "\n").
		$comment."## START EasySCP ".$zone." ###\n".
		$data."\n".
		$comment."## END EasySCP ".$zone." ###\n".
		$az;
}
...
sub connect_easyscp_daemon {

	push_el(\@main::el, 'connect_easyscp_daemon()', 'Starting...');
	...
	if (!defined $fd) {
		...
		push_el(
			\@main::el,
			'connect_easyscp_daemon()',
			"[ERROR] Can't connect to EasySCP license daemon !"
		);

		return (-1, '');
	}

	push_el(\@main::el, 'connect_easyscp_daemon()', 'Ending...');
...
sub close_easyscp_daemon {

	push_el(\@main::el, 'close_easyscp_daemon()', 'Starting...');

	my ($fd) = @_;

	close($fd);

	push_el(\@main::el, 'close_easyscp_daemon()', 'Ending...');
}
...
sub license_request {
	...
	my ($rs, $rdata) = connect_easyscp_daemon();
...
# Default value is the main EasySCP configuration file (easyscp.conf)
# @return int 0 on success, 1 otherwise
#
sub get_conf {
...
################################################################################
# Store all cached configuration parameters in the easyscp.conf file
#
...
";
	...
	$out -> build(
		...
		To => $admin_email,
		Subject => "[$date] EasySCP Error report",
		Data => $msg_data,
		'X-Mailer' => "EasySCP $main::cfg{'Version'} Automatic Error Messenger"
	);
...
sub write_easyscp_key_cfg {

	push_el(\@main::el, 'write_easyscp_key_cfg()', 'Starting...');
	...
	if ($rs != 0) {
		push_el(
			\@main::el,
			'write_easyscp_key_cfg()', '[ERROR] Failed to create file $main::key_conf!'
		);
		
		return -1;
	}

	push_el(\@main::el, 'write_easyscp_key_cfg()', 'Ending...');
...
sub read_easyscp_key_cfg {

	push_el(\@main::el, 'read_easyscp_key_cfg()', 'Starting...');
	...
	push_el(\@main::el, 'read_easyscp_key_cfg()', 'Ending...');
